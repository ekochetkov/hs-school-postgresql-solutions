    ...
    (
      ...
      FROM
        ...
        SELECT
          id,
          path,
          CASE WHEN jsonb_typeof( resource ) = 'array' THEN resource
                ELSE jsonb_build_array( resource )
            END resource_allways_array
        FROM keys
        WHERE
          (jsonb_typeof( resource ) = 'array' AND jsonb_typeof( resource->0 ) = 'object' )
            OR
          jsonb_typeof( resource ) = 'object'
        ) t
      , jsonb_array_elements_text(resource_allways_array) jaet(array_element_iter_text)
      , jsonb_each(array_element_iter_text::jsonb) x(k, v)
    ), total AS
      (
        SELECT COUNT(*) ttl FROM "test_patient"
      )
    
    SELECT path, COUNT(*), ROUND(COUNT(*)*100/ttl::FLOAT) persent
      FROM keys, total
    GROUP BY path, ttl
